...
import { highRateLimit, mediumRateLimit } from '../middleware/rateLimit';
import { requirePropertyOwnership, requirePropertyAccess } from '../middleware/authorization';
import { validateBody } from '../middleware/validation';
import { updatePropertySchema, propertyAvailabilitySchema, propertyLawyerSchema, propertyProgressSchema } from '../lib/validation/schemas';
...
/** Resolve property image entry to full URL (local path → absolute, S3 key → signed URL) */
async function resolveImageUrl(entry: string): Promise<string> {
  if (!entry || typeof entry !== 'string') return '';
  const t = entry.trim();
  if (!t) return '';
  if (t.startsWith('http://') || t.startsWith('https://')) return t;
  if (t.startsWith('/uploads/')) return `${BACKEND_BASE.replace(/\/$/, '')}${t}`;
  if (s3Client && process.env.AWS_S3_BUCKET && (t.startsWith('properties/') || !t.startsWith('/'))) {
    try {
...
async function resolvePropertyImages(images: string[]): Promise<string[]> {
...
const uploadImages = createSecureUpload('image', 10); // Max 10 images
const uploadSingleImage = createSecureUpload('image', 1);
...
async function uploadFiles(files: Express.Multer.File[]): Promise<string[]> {
  ...
  try {
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      try {
        // Scan for malware (stub for now)
        const scanResult = await scanForMalware(file.buffer, file.filename || file.originalname);
        if (!scanResult.clean) {
          console.error(`Malware detected in file: ${file.originalname}`, scanResult.threat);
        ...
        if (s3Client && process.env.AWS_S3_BUCKET) {
          // Upload to S3 with secure filename
          const s3Key = `properties/${secureFileName}`;
          await s3Client.send(new PutObjectCommand({
            Bucket: process.env.AWS_S3_BUCKET,
            Key: s3Key,
            Body: file.buffer,
            ContentType: file.mimetype,
            // ACL not set = defaults to private (bucket policy should block public access)
            // Ensure bucket has "Block Public Access" enabled in AWS console
          }));
          // DO NOT return direct S3 URL - client must request signed URL via /api/files/download-url
          // Store S3 key for later signed URL generation
          photoUrls.push(s3Key);
        } else {
          // Upload to local storage with secure filename
          ...
          const fileUrl = `/uploads/properties/${secureFileName}`;
          photoUrls.push(fileUrl);
        }
      } catch (uploadError) {
        console.error('Error uploading photo:', uploadError);
        // Don't add placeholder - fail securely
      }
    }
  } catch (error) {
...
router.get('/', highRateLimit, optionalAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [properties, total] = await Promise.all([
      prisma.property.findMany(query),
      prisma.property.count({ where: query.where }),
    ]);

    const propertyIds = properties.map(p => p.id);

    // Sold: COMPLETED or CLOSED deals
    ...
    // Deposit completed: ACTIVE deal with lawyerApprovedBasicDocumentsAt = property locked for that deal
    ...
    const dataWithSold = await Promise.all(
      properties.map(async (p) => {
        const images = await resolvePropertyImages(p.images || []);
        const sold = soldPropertyIds.has(p.id) || (p as any).isSold;
        const depositLocked = !sold && depositLockedPropertyIds.has(p.id);
        return {
          ...p,
          images,
          propertySold: sold,
...
router.post('/', mediumRateLimit, optionalAuth, uploadImages.array('photos'), validateUploadedFile('image'), async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    auditLogger.propertyCreate(req, property.id);
...
router.get('/:id', optionalAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const soldCount = await prisma.dealRoom.count({
      where: {
        propertyId: property.id,
        status: { in: ['COMPLETED', 'CLOSED', 'CLOSED_PROPERTY_SOLD'] },
      },
    });
    const propertySold = soldCount > 0;

    const depositLockedCount = await prisma.dealRoom.count({
      where: {
        propertyId: property.id,
        status: 'ACTIVE',
        lawyerApprovedBasicDocumentsAt: { not: null },
    ...
    const depositLocked = !propertySold && depositLockedCount > 0;

    const images = await resolvePropertyImages(property.images || []);

    res.json({ property: { ...property, images, propertySold, depositLocked } });
  } catch (error) {
...
router.patch('/:id', optionalAuth, requirePropertyOwnership, validateBody(updatePropertySchema), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.delete('/:id', optionalAuth, requirePropertyOwnership, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    auditLogger.propertyDelete(req, id);

    res.json({ success: true });
  } catch (error) {
    console.error('Error deleting property:', error);
    const { id: propertyId } = req.params;
    auditLogger.apiError(req, error instanceof Error ? error : new Error('Unknown error'), `/properties/${propertyId}`);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
// POST /api/properties/:id/view - Record property view (counts for seller overview "Προβολές")
router.post('/:id/view', optionalAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Always increment PropertyStats.views (for seller overview modal)
    await prisma.propertyStats.upsert({
      where: { propertyId: id },
    ...
    // If logged in, also record in PropertyView (who viewed, when)
    if (userId) {
...
router.post('/images', mediumRateLimit, optionalAuth, uploadSingleImage.single('file'), validateUploadedFile('image'), async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    if (s3Client && process.env.AWS_S3_BUCKET) {
      ...
      fileUrl = `/uploads/properties/${secureFileName}`;
    }
...
router.post('/:id/availability', optionalAuth, requirePropertyOwnership, validateBody(propertyAvailabilitySchema), async (req: AuthRequest, res: Response) => {
  try {
    const propertyId = req.params.id;
...
router.delete('/:id/availability', optionalAuth, requirePropertyOwnership, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Verify availability belongs to property
    const availability = await prisma.propertyAvailability.findUnique({
      where: { id: availabilityId },
      select: { propertyId: true },
    });

    if (!availability || availability.propertyId !== req.params.id) {
...
router.post('/:id/lawyer', optionalAuth, requirePropertyOwnership, async (req: AuthRequest, res: Response) => {
  try {
    const propertyId = req.params.id;

    const { name, email, phone, taxId, barAssociation } = req.body;
...
// GET /api/properties/:id/engineer - Get engineer info for property (seller's custom engineer)
router.get('/:id/engineer', optionalAuth, async (req: AuthRequest, res: Response) => {
  try {
    const propertyId = req.params.id;

    const property = await prisma.property.findUnique({
      where: { id: propertyId },
      select: { engineerInfo: true },
    });
    ...
    res.json(property.engineerInfo || {});
  } catch (error) {
    console.error('Error fetching engineer info:', error);
    res.status(500).json({ error: 'Σφάλμα κατά την ανάκτηση των στοιχείων μηχανικού.' });
  }
});

// POST /api/properties/:id/engineer - Save engineer info for property (seller's custom engineer)
router.post('/:id/engineer', optionalAuth, requirePropertyOwnership, async (req: AuthRequest, res: Response) => {
  try {
    const propertyId = req.params.id;
    ...
    console.error('Error saving engineer info:', error);
    res.status(500).json({ error: 'Σφάλμα κατά την αποθήκευση των στοιχείων μηχανικού.' });
  }
});
...
router.post('/:id/progress/documents', optionalAuth, requirePropertyOwnership, uploadDocument.single('file'), validateUploadedFile('document'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({ 
      success: true, 
      s3Key, // Client will use this to request signed URL via /api/files/download-url?key=<s3Key>
      message: 'File uploaded successfully. Use /api/files/download-url?key=<s3Key> to get signed URL.'
    });
  } catch (error) {
...
router.post('/:id/request-removal', optionalAuth, requirePropertyOwnership, async (req: AuthRequest, res: Response) => {
  try {
    const propertyId = req.params.id;
...
router.put('/:id/progress', optionalAuth, requirePropertyOwnership, validateBody(propertyProgressSchema), async (req: AuthRequest, res: Response) => {
  try {
