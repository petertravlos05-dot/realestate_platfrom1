    ...
    if (activeFilters) {
      ...
      if (activeFilters.priceRange?.min || activeFilters.priceRange?.max) {
        ...
        filtered = filtered.filter(property => {
          ...
          const meetsMinPrice = price >= minPrice;
          const meetsMaxPrice = price <= maxPrice;
          console.log('Property price:', price, 'Meets min:', meetsMinPrice, 'Meets max:', meetsMaxPrice);
      ...
      if (activeFilters.propertyType && activeFilters.propertyType.length > 0) {
        console.log('Property types to filter:', activeFilters.propertyType);
        filtered = filtered.filter(property => {
          const matches = activeFilters.propertyType.includes(property.type);
          console.log('Property type:', property.type, 'Matches:', matches);
          return matches;
        });
      ...
      if (activeFilters.bedrooms && activeFilters.bedrooms !== '-') {
        const minBedrooms = parseInt(activeFilters.bedrooms);
        console.log('Filtering bedrooms >=', minBedrooms);
        filtered = filtered.filter(property => {
          const bedrooms = Number(property.bedrooms);
          const matches = bedrooms >= minBedrooms;
          console.log('Property bedrooms:', bedrooms, 'Matches:', matches);
          return matches;
        });
      ...
      if (activeFilters.bathrooms && activeFilters.bathrooms !== '-') {
        const minBathrooms = parseInt(activeFilters.bathrooms);
        console.log('Filtering bathrooms >=', minBathrooms);
        filtered = filtered.filter(property => {
          const bathrooms = Number(property.bathrooms);
          const matches = bathrooms >= minBathrooms;
          console.log('Property bathrooms:', bathrooms, 'Matches:', matches);
          return matches;
        });
      ...
      if (activeFilters.area?.min || activeFilters.area?.max) {
        const minArea = activeFilters.area.min ? Number(activeFilters.area.min) : 0;
        const maxArea = activeFilters.area.max ? Number(activeFilters.area.max) : Infinity;
        console.log('Area range:', { minArea, maxArea });
        
        filtered = filtered.filter(property => {
          const area = Number(property.area);
          const meetsMinArea = area >= minArea;
          const meetsMaxArea = area <= maxArea;
          console.log('Property area:', area, 'Meets min:', meetsMinArea, 'Meets max:', meetsMaxArea);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <main className="pt-16">
        ...
        <motion.div 
          ...
          className="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 overflow-hidden min-h-screen flex items-center justify-center"
        >
          {/* Background Pattern */}
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      >
                        ...
                        {activeFilters && (
                          <motion.span 
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="ml-2 px-3 py-1 bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-sm rounded-full font-medium"
                          >
                    ...
                    <motion.button
                      ...
                      className="flex items-center justify-center px-8 py-4 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl hover:from-blue-600 hover:to-indigo-600 transition-all duration-300 shadow-lg font-medium text-lg"
                    >
        ...
        <motion.div 
          ...
          className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-gradient-to-b from-white via-blue-50 to-white rounded-3xl shadow-xl mt-[-4rem] relative z-10"
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-4 py-2 rounded-lg border border-gray-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 text-gray-700"
              >
            ...
            {viewMode === 'map' ? (
                ...
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-2 mt-10">
                    <button
                      ...
                      className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-full shadow-lg p-3 transition-all duration-300 hover:scale-110 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              >
                <div className="max-w-md mx-auto">
                  <div className="w-28 h-28 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg">
                    <FaSearch className="w-16 h-16 text-blue-400" />
                  </div>
                  ...
                  {hasSearched && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={handleReset}
                      className="px-8 py-4 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl hover:from-blue-600 hover:to-indigo-600 transition-all duration-300 shadow-lg text-lg font-semibold"
                    >
                      Επαναφορά Φίλτρων
                    </motion.button>
                  )}
