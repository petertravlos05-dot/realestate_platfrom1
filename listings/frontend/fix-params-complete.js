/**
 * Complete script to fix ALL missing await params in Next.js 16 route handlers
 * Strategy: For each file with params.xxx, add await params at start of each function, then replace all params.xxx
 */
...
function fixRouteFile(filePath) {
  ...
  // Find all async function definitions - match any function with params in signature
  ...
  for (let i = functionMatches.length - 1; i >= 0; i--) {
    ...
    // Find where to insert (right after opening brace)
    ...
    // Get indentation - look at first line after brace
    let indentPos = openingBrace + 1;
    while (indentPos < content.length && indentPos < openingBrace + 100 && content[indentPos] !== '\n' && /\s/.test(content[indentPos])) {
      indentPos++;
    }
    const indentMatch = content.substring(openingBrace + 1, indentPos + 20).match(/^(\s+)/);
    const indent = indentMatch ? indentMatch[1] : '    ';
    
    // Check if await params already exists - look for pattern: const { ... } = await params;
    const functionBodyStart = insertPos;
    const nextBrace = content.indexOf('}', functionBodyStart);
    const functionBodyEnd = nextBrace !== -1 ? nextBrace : Math.min(functionBodyStart + 500, content.length);
